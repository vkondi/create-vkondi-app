...
describe('Next.js Project Scaffold', () => {
  ...
  describe('scaffoldNext', () => {
    ...
    it('should detect TypeScript from tsconfig.json', async () => {
      ...
      expect(vi.mocked(pathExists)).toHaveBeenCalledWith(expect.stringContaining('tsconfig.json'));
    });
    ...
    it('should configure Next.js config with security headers', async () => {
      ...
      const nextConfigCall = vi
        .mocked(writeFile)
        .mock.calls.find((call) => call[0].includes('next.config'));
    ...
    it('should log success after TypeScript configuration', async () => {
      ...
      expect(calls.some((call) => call[0] === 'Strict TypeScript configuration applied')).toBe(
        true
      );
    });
